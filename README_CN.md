...
Brainless 在 `settings.json` 中安装 **4 个 Hook**，覆盖 Claude Code 会话的完整生命周期。这是真正写入配置的自动化 —— **不依赖提示词指令，即使上下文被压缩也能正常工作。**

| Hook 事件 | 脚本 | 功能 |
|-----------|------|------|
| **SessionStart** | `session_start.py` | 会话开始时注入大脑上下文 —— 总条目数、当前项目已知问题（按 cwd/repo 匹配）、最近条目 |
| **PostToolUse** (Bash) | `bash_error_search.py` | 命令失败时自动搜索知识库，追踪未匹配的新错误 |
| **PostToolUse** (Edit\|Write) | `post_tool_logger.py` | 记录文件编辑活动，检查修改的文件是否关联已知 KB 条目 |
| **Stop** | `session_end.py` | 记录会话摘要（时长、工具次数、大脑命中），**提醒未记录的新错误**需要 `/brain-dump` |

```
SessionStart                   PostToolUse(Bash)               Stop
    |                                |                            |
    v                                v                            v
 加载 _cache.json              出错？搜索 _cache.json         读取 _session_errors.json
 按 cwd/repo 搜索项目已知问题    |-- 匹配 → 展示解决方案       |-- 有未记录错误 → 提醒 /brain-dump
 展示项目特定的踩坑记录         |-- 无匹配 → 追踪错误         |-- 记录时长/工具/命中数
 重置会话追踪                   v                              清理临时文件
...
```
 ...
 加载大脑上下文                  搜索 _cache.json                 检查未记录的错误
 展示项目已知问题                 /        \                      提醒: /brain-dump
     |                         是         否                     记录会话摘要
     v                          |           |
 Claude 带着完整                应用方案   正常调试 → 解决了？
 大脑意识开始工作                          → 自动记录
                                          → 追踪到会话结束
```
...
└── brainless/                   # 知识库数据
    ...
    ├── _cache.json              # 快速搜索缓存
    ├── hooks/
    │   ├── session_start.py     # SessionStart hook — 上下文注入
    │   ├── bash_error_search.py # PostToolUse(Bash) — 错误自动搜索
    │   ├── post_tool_logger.py  # PostToolUse(Edit|Write) — 活动日志
    │   └── session_end.py       # Stop hook — 会话摘要 + 提醒
    └── <13 个分类目录>/          # 分类子索引 + 条目
```
...
安装脚本注册 **4 个生命周期 Hook**（SessionStart、PostToolUse/Bash、PostToolUse/Edit|Write、Stop）。这些 Hook 运行 Python 脚本，提供不依赖提示词的真正自动化 —— 会话开始时注入上下文、命令失败时自动搜索、文件编辑时记录活动、会话结束时汇总并提醒未记录的新错误。安装脚本会安全地合并 hook 到你现有的 settings 中，不会覆盖任何已有配置。
